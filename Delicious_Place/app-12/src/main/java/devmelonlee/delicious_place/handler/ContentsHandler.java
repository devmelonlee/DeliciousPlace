...
public class ContentsHandler {
  ...
  public static void inputContents() { // 1. 리뷰 등록
    ...
    c.setId(Prompt.inputString("아이디를 입력해주세요 : "));
    c.setEmail(Prompt.inputString("이메일을 입력해주세요 : "));
    c.setPassword(Prompt.inputString("암호를 입력해주세요 : "));
    c.setGender(inputGender((char)0)); // 기본값 : 0, 명시적 형변환

    c.setStoreName(Prompt.inputString("리뷰할 가게 이름을 적어주세요 : "));
    c.setContent(Prompt.inputString("리뷰 내용을 적어주세요 : "));
    c.setIsReceipt(inputReceipt((char)0)); // 기본값 : 0, 명시적 형변환

    while (true) { 
      int validStarRaiting = Prompt.inputInt("평점은요 (1, 2, 3, 4, 5)? : ");
      if (validStarRaiting >= 1 && validStarRaiting <= 5) {
        c.setStarRaiting(validStarRaiting);
        break;
      }
  ...
  public static void printContents() { // 2. 전체 리뷰 조회
    ...
    for (int i = 0; i < length; i++) {
      ...
      System.out.printf("# %05d,  %s,  %s, %s \n", 
        c.getNo(), c.getId(), c.getEmail(), toGenderString(c.getGender()));
      System.out.println("가게 이름   : " + c.getStoreName());
      System.out.println("리뷰 내용   : " + c.getContent());
      System.out.println("영수증 인증 : "+ c.getIsReceipt());
      System.out.print("별점        : ");
      for (int j = 1; j <= c.getStarRaiting(); j++) {
        System.out.print("⭐️");
      }
      System.out.println("");
      System.out.println("---------------------------------------------");
  ...
  public static void viewContents() { // 3. 리뷰 글 번호 조회
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (c.getNo() == Integer.parseInt(contentNo)) {
        System.out.println("---------------------------------------------");
        System.out.printf(" 글 번호 |   아이디   |   이메일  |   성별  \n");
        System.out.println("---------------------------------------------");
        System.out.printf("# %05d, %s, %s, %s \n", 
  ...
  public static void updateContents() { // 4. 리뷰 수정하기
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (c.getNo() == Integer.parseInt(contentNo)) {
        ...
        c.setStoreName(Prompt.inputString(""));
        System.out.printf("글 내용을 변경하시겠습니까?\n (현재 : %s) : ", c.getContent());
        c.setContent(Prompt.inputString(""));
        System.out.printf("영수증 여부를 변경하시겠습니까?\n : ");
        c.setIsReceipt(inputReceipt(c.getIsReceipt()));
        return;
      }
    }
    System.out.println("해당 번호의 글은 없습니다!");
  ...
  public static void deleteContents() { // 5. 리뷰 삭제
    ...
    // if (passwordValidation(contents[i], )) {
   ...
   // 추가
  public static Boolean passwordValidation(String[] str1, String[] str2) {
